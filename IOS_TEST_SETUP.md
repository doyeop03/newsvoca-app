...
The repository includes the iOS `GoogleService-Info.plist` for Firebase project
`newswordapp-7a53b` and bundle identifier `com.example.wordapp`.

1. Confirm it exists at `ios/Runner/GoogleService-Info.plist` after cloning.
...
The file is Firebase iOS client configuration for this private repository. It must never be
replaced with a service-account JSON, private key, client secret, or backend credential.
...
`lib/firebase_options.dart`, and `ios/Runner/GoogleService-Info.plist`. Build outputs such as
`build/`, `.dart_tool/`, `ios/Pods/`, and Xcode DerivedData are not required.
